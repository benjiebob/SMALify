...
parser.add_argument('--mesh_dir', type=str, default='fitter_3d/example_meshes', help="Directory (relative to SMALify) in which meshes are stored")
...
def main(args):
	...
	# if provided, load scheme from YAML
	if args.yaml_src is not None:
		try:
			with open(args.yaml_src) as infile:
				schemes = yaml.load(infile, Loader=yaml.FullLoader)
		except FileNotFoundError:
			raise FileNotFoundError("No YAML file found at {args.yaml_src}. Make sure this is relative to the SMALify directory.")

		for stage_name, kwargs in schemes['stages'].items():
